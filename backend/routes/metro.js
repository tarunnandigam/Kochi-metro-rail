...
router.post('/book', async (req, res) => {
    try {
        ...
        const bookingObj = { bookingId, fromStation: fromCode || '', toStation: toCode || '', passengerName, passengerPhone, fare, createdAt: new Date() };
        ...
        try {
            ...
            // Save to fallback file so ticket generation still works
            saveBookingFallback(bookingObj);
            savedBooking = bookingObj;
        }

        // If user is authenticated (Bearer token), attach booking to their account
        try {
                ...
                try {
                    ...
                    if (userId) {
                        ...
                        if (u) {
                            ...
                            u.bookings.push({ bookingId, type, fare, createdAt: new Date() });
                            await u.save();
                        }
                    }
                } catch (e) {
            ...
            console.warn('Failed to attach booking to user:', e && e.message);
        }
